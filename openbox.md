```
apt-get -y install xrdp openbox x11-xserver-utils python3-xdg pipewire libsecret-1-0 libvte-2.91-0 menu thunar tint2 yaru-theme-icon chromium-browser mesa-utils
```
```
tee -a /etc/bash.bashrc << 'RCEOF'
# VTE shell integration with Tilix
if [ "${TILIX_ID}" ] || [ "${VTE_VERSION}" ]; then
  _vte_script=$(ls /etc/profile.d/vte-*.sh 2>/dev/null | sort -V | tail -n1)
  [ -f /etc/profile.d/vte.sh ] && source /etc/profile.d/vte.sh || { [ -n "${_vte_script}" ] && source "${_vte_script}"; }
  unset _vte_script
fi
RCEOF
```
```
mkdir -vp /home/${DEFAULT_USER}/.config/openbox
echo 'tint2 &' >> /home/${DEFAULT_USER}/.config/openbox/autostart
```
```
DEFAULT_USER=$(ls /home | head -n 1)
```
```
chown -R ${DEFAULT_USER}:${DEFAULT_USER} /home/${DEFAULT_USER}
```
